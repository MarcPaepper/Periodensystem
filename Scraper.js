...
async function retrieveLengthsSequentially() {
    ...
    // save to output file Output/Wikiviews.json
    
    fs.writeFileSync('Output/Wikiviews.json', JSON.stringify(jsonData, null, 2), 'utf-8');
...
async function getPageViews(name) {
    ...
    const apiUrl = `https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/en.wikipedia/all-access/all-agents/${name}/monthly/20220101/20221231`;
    ...
    try {
		...
		let length = `${name}${views}`.length;
		console.log(`${name}: ${' '.repeat(20-length)}${views}`);
